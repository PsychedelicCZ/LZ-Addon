    ...
    // @version      1.1.2
    // @description  Gladiatus Addon - Legie Znovuzrozených
    // @author       Psyche
    // @match        *://*/*
    ...
    `;
            ...
            function formatDate(dateString) {
                ...
                const czechDate = new Date(utcDate.getTime() - 2 * 60 * 60 * 1000);

                return czechDate.toLocaleString('cs-CZ', {
                    day: '2-digit',
                    month: '2-digit',
                    year: 'numeric',
                    hour: '2-digit',
            ...
            function getTimeRemaining(targetDate) {
                const now = new Date();
                const utcTarget = new Date(targetDate);
                const czechTarget = new Date(utcTarget.getTime() - 2 * 60 * 60 * 1000);
                const diff = czechTarget - now;
            ...
            function isEventActive(start, end) {
                ...
                const czechStart = new Date(utcStart.getTime() - 2 * 60 * 60 * 1000);
                const czechEnd = new Date(utcEnd.getTime() - 2 * 60 * 60 * 1000);

                return now >= czechStart && now <= czechEnd;
            }
            ...
            function displayEvents(events) {
                ...
                const now = new Date();
                const sortedEvents = events.sort((a, b) => new Date(a.start) - new Date(b.start));            let html = '<div style="color: #BFAE54; font-weight: bold; margin-bottom: 12px; text-align: center; font-size: 14px;">📅 Gladiatus Eventy</div>';


            html += `<div style="color: #888; font-size: 10px; text-align: center; margin-bottom: 0px;"></div>`;
            ...
            sortedEvents.forEach((event, index) => {
                ...
                const debugInfo = `UTC: ${event.start} → ${event.end}`;
                ...
                if (isActive) {
                    ...
                    html += `
                        <div style="
                            margin-bottom: ${index < sortedEvents.length - 1 ? '5px' : '0'};
                            padding: 10px;
